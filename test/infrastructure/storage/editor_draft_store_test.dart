...
void main() {
  ...
  test('暫存附件為加密檔，materialize 後可讀且清理會移除明文', () async {
    ...
    final File sourceFile = File(p.join(rootDir.path, 'picked.txt'));
  ...
  test('暫存附件、寫入草稿後再刪除會移除檔案', () async {
    ...
    final EditorDraftRecord record = EditorDraftRecord(
      ...
      tags: const <String>[],
      markdownBody: '',
      attachmentIds: const <String>['pending-1'],
      pendingAttachments: <EditorDraftPendingAttachment>[
  ...
  test('寫入時會清理未引用的暫存檔', () async {
    final File sourceA = File(p.join(rootDir.path, 'a.txt'))
      ..writeAsStringSync('a');
    final File sourceB = File(p.join(rootDir.path, 'b.txt'))
      ..writeAsStringSync('b');
    ...
    final EditorDraftRecord record = EditorDraftRecord(
      ...
      tags: const <String>[],
      markdownBody: '',
      attachmentIds: const <String>['pending-a'],
      pendingAttachments: <EditorDraftPendingAttachment>[
        EditorDraftPendingAttachment(
          assetId: 'pending-a',
  ...
  test('stagePendingFile 空路徑或不存在來源會拋錯', () async {
    ...
    await expectLater(
      store.stagePendingFile(
        'entry-1',
        p.join(rootDir.path, 'missing.txt'),
        session,
      ),
