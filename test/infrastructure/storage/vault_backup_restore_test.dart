...
void main() {
  ...
  group('peekBackupRecovery', () {
    test('可讀取備份內的 recovery.json', () async {
  ...
  group('inspectBackup', () {
    ...
    test('會拒絕可攜式 markdown 匯出 zip', () async {
      final setup = await harness.repository.setupRecoveryKey();
      await harness.saveSimpleEntry(
        setup,
        title: 'Export Entry',
    ...
    test('會拒絕損壞的 recovery.json', () async {
    ...
    test('會拒絕缺少 recovery.json 的 zip', () async {
      final File backupFile = File(
  ...
  group('restoreBackupZip', () {
    test('覆寫前會拒絕缺少加密資料的備份', () async {
      final setup = await harness.repository.setupRecoveryKey();
    ...
    test('可還原日記並保留 recovery metadata', () async {
      final setup = await harness.repository.setupRecoveryKey();
    ...
    test('還原後會清除 materialize 的暫時明文附件', () async {
      final setup = await harness.repository.setupRecoveryKey();
      await harness.saveSimpleEntry(
        setup,
        title: 'Restore Me',
