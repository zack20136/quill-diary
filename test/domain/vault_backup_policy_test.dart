...
void main() {
  group('VaultBackupPolicy', () {
    test(
      'backupFileName formats timestamp as backup_YYYY-MM-DD_HH-MM-SS.zip',
      () {
        expect(
    ...
    test('hasVaultBackupExtension accepts zip case insensitively', () {
      ...
      expect(
        VaultBackupPolicy.hasVaultBackupExtension('my_diary_backup.ZIP'),
        isTrue,
      );
    });
    ...
    test('hasVaultBackupExtension rejects non-zip extensions', () {
      ...
      expect(
        VaultBackupPolicy.hasVaultBackupExtension('backup.jbackup'),
        isFalse,
      );
    });

    test(
      'markdownPortableFileName formats timestamp as markdown_YYYY-MM-DD_HH-MM-SS.zip',
      () {
        expect(
          VaultBackupPolicy.markdownPortableFileName(
            DateTime(2026, 5, 26, 14, 3, 7),
          ),
    ...
    test(
      'htmlPortableFileName formats timestamp as html_YYYY-MM-DD_HH-MM-SS.html',
      () {
        expect(
          VaultBackupPolicy.htmlPortableFileName(
            DateTime(2026, 5, 26, 14, 3, 7),
          ),
