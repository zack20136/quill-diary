...
void main() {
  ...
  test('預設使用 plain keystore 保護 trusted session（無解鎖模式）', () async {
    ...
    expect(
      session.deviceSlotId,
      'dev_android_keystore_plain_${session.vaultId}',
    );
  ...
  test(
    '開啟生物驗證後，refreshTrustedSessionProtection 會切到 biometric trusted session',
    () async {
      final RecoverySetupResult setup = await harness.repository
          .setupRecoveryKey();
      await harness.appLockService.setUnlockMode(AppUnlockMode.biometric);

      final UnlockedVaultSession refreshed = await harness.repository
          .refreshTrustedSessionProtection(
            setup.session,
            authKind: KeystoreAuthKind.biometric,
          );
      final WrappedRecoveryKeyRecord? record = await harness.deviceKeyManager
          .readWrappedRecoveryKey(refreshed.vaultId);

      expect(
        harness.deviceKeyManager.lastEnsureAuthKind,
      ...
      expect(
        refreshed.deviceSlotId,
        'dev_android_keystore_biometric_${refreshed.vaultId}',
      );
      expect(
        record?.slotId,
        'dev_android_keystore_biometric_${refreshed.vaultId}',
