...
void main() {
  ...
  test(
    'saveBackupToAppLocal writes a healthy backup under app backups directory',
    () async {
      ...
      final BackupPersistResult result = await transferService
          .saveBackupToAppLocal();
      final Directory backupsDirectory = await harness.pathStrategy
  ...
  test('saveBackupToAppLocal does not persist when inspect fails', () async {
    ...
    final int beforeCount = backupsDirectory
        .listSync()
        .whereType<File>()
        .length;
  ...
  test(
    'listAppLocalBackups sorts newest first and deleteAppLocalBackup removes the file',
    () async {
      final Directory backupsDirectory = await harness.pathStrategy
          .localBackupsDirectory();
      await backupsDirectory.create(recursive: true);
      ...
      final List<LocalBackupFile> backups = await transferService
          .listAppLocalBackups();
  ...
  test(
    'listAppLocalBackups includes zip files without backup_ prefix',
    () async {
      final Directory backupsDirectory = await harness.pathStrategy
          .localBackupsDirectory();
      await backupsDirectory.create(recursive: true);
      File(
        p.join(backupsDirectory.path, 'my_diary_backup.zip'),
      ).writeAsBytesSync(const <int>[1]);
  ...
  test('listAppLocalBackups keeps only the newest five backups', () async {
    ...
    for (int index = 0; index < 6; index++) {
      final File backup = File(
        p.join(backupsDirectory.path, 'backup_$index.zip'),
    ...
    expect(
      File(p.join(backupsDirectory.path, 'backup_0.zip')).existsSync(),
      isFalse,
    );
  });
}
