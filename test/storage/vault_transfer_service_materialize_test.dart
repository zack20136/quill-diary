...
void main() {
  ...
  group('PickedFileMaterializer', () {
    test('cached path 可用時不觸發 URI 與 bytes', () async {
      ...
      materializer = createMaterializer(
        copyAndroidUriToPath: ({required sourceUri, required destinationFile}) async {
          uriCalled = true;
        },
    ...
    test('還原 cached path 時會 copy 到 app temp 且 shouldDeleteAfterUse 為 true', () async {
      final File source = File('${workDir.path}/restore.quillvault');
      await source.writeAsBytes(<int>[1]);

      final MaterializedPickedFile materialized = await materializer.materialize(
        PlatformFile(name: 'restore.quillvault', size: 1, path: source.path),
      ...
      final MaterializedPickedFile result = await materializer.materialize(
        AndroidPlatformFile(
          ...
          safHandle: AndroidSAFHandle(
            uri: Uri.parse('content://com.android.providers.downloads.documents/document/123'),
  ...
  group('bytesFallback 保底', () {
    test('小於 64 MiB 的 bytes 可寫入 temp', () async {
      ...
      expect(await result.file.readAsBytes(), Uint8List.fromList(<int>[1, 2, 3, 4]));
      await result.file.delete();
    });
