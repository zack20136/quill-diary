...
void main() {
  ...
  test('homeEntriesProvider 空搜尋時沿用全量索引快取並排序', () async {
    ...
    final List<EntryIndexRecord> allEntries = await container.read(
      allEntryIndexRecordsProvider.future,
    );
    final List<EntryIndexRecord> homeEntries = await container.read(
      homeEntriesProvider.future,
    );
  ...
  test('日曆 provider 從 repository 取得日期與月份資料', () async {
    ...
    final List<EntryIndexRecord> dayEntries = await container.read(
      calendarEntriesProvider.future,
    );
    final List<DateOnly> monthDates = await container.read(
      calendarMonthEntryDatesProvider.future,
    );
    final List<EntryIndexRecord> monthEntries = await container.read(
      calendarMonthEntriesProvider.future,
    );
    ...
    expect(repository.listEntriesCalls, 1);
    expect(repository.listEntriesDates, <DateOnly?>[
  ...
  test('memoryEntriesProvider 的月份範圍直接查詢 repository 月資料', () async {
    ...
    final List<EntryIndexRecord> entries = await container.read(
      memoryEntriesProvider.future,
    );

    expect(entries.map((EntryIndexRecord entry) => entry.id), <String>[
      'jrn_MAY_NEWER',
      'jrn_MAY',
    ]);
    expect(repository.listEntriesForMonthCalls, 1);
