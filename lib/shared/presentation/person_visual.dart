...
Color personAvatarBackgroundColor(Person person, Color surface) =>
    Color.alphaBlend(
      personAccentColor(person).withValues(alpha: 0.18),
      surface,
    );
...
Color defaultPersonAccentColor(PersonId personId) {
  if (personId.isEmpty || kDefaultTagAccentPresets.isEmpty) {
    return kDefaultTagAccentPresets.first;
  }
  final int hash = personId.codeUnits.fold<int>(
    0,
    (int acc, int unit) => (acc * 31 + unit) & 0x7fffffff,
  );
  return kDefaultTagAccentPresets[hash % kDefaultTagAccentPresets.length];
}
