...
Quill Diary 是以 Android 為主要平台的離線加密日記 App。本 repo 提供原始碼、開發入口與公開文件。
...
- Google Drive `appDataFolder` 備份與還原；由使用者手動啟動，不是即時同步
- Google Play 一次性支持，不提供訂閱、會員或額外功能解鎖

目前僅支援 Android。`linux/`、`macos/`、`windows/` 與 `web/` 仍保留 Flutter 平台骨架，但不在支援範圍內。
...
- 原始碼以 [GNU Affero General Public License v3.0 or later](LICENSE)（AGPL-3.0-or-later）發布。若您修改並發布本程式，需依授權條款提供對應完整原始碼。
