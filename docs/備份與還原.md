...
2. `peekBackupRecovery` 讀取備份內 `recovery.json`（不寫入磁碟）
3. `precheckRestore` 比對本機 `vault_id` 與是否仍有 trusted device
4. 顯示確認對話框（覆寫警告、是否需復原金鑰、末四碼提示等）
5. 使用者確認後才執行還原
