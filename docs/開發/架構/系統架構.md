...
[`vault_transfer_service.dart`](../../../lib/infrastructure/storage/vault_transfer_service.dart) 是備份、還原、匯入匯出與外部交付的協調入口。

主要責任：

- 建立完整備份 zip 並先做 inspect
- 寫入 app local backups
- 寫到外部資料夾
- 上傳 / 下載 Google Drive 備份
- portable markdown / html 匯入匯出
- backup restore precheck 與 restore 實際呼叫
...
4. `VaultTransferService` 再建立在 archive + drive + external delivery 之上
...
flowchart TD
  ...
  G --> J["Vault files"]
  K["SettingsFlowController"] --> L["VaultTransferService"]
  L --> M["VaultArchiveIo"]
