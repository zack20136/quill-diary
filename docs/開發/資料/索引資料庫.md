...
- 初始化會就地補上可安全新增的 `preview_markdown`、`body_visible_text` 欄位與人物分析表，保留既有索引列；其他不相容 schema 才重置。
...
| `people_analysis_documents` | 人物分析文件狀態；以日記內容 hash 判斷哪些索引列需要重新分析，包含零命中的日記 |
| `entry_people_analytics` | 人物姓名查詢的加密衍生快取（`entry_id`/`person_id`/`entry_date`）；只在讀取人物功能時依索引文字延遲更新，不屬於日記資料模型 |
| `app_kv` | 索引內部狀態與同步旗標 |
...
- `body_visible_text`：通用可見正文；排除 fenced code、inline code、圖片網址與連結目的地，供按需衍生查詢使用
...
- 通用 Markdown 索引抽取器會在同一次走訪中產生搜尋正文、可見正文與預覽文字；搜尋正文會移除 task checkbox 等標記，但保留可搜尋內容
...
| `people_analytics_generation` | 人物姓名比對規則版本；人物功能初版為 `1` |
| `people_analytics_catalog_fingerprint` | 最近分析使用的姓名/別名集合 fingerprint |
| `people_analytics_stale` | 分析是否尚未完整完成；中斷後依文件 hash 接續 |
...
初始化前缺少 `preview_markdown` 或 `body_visible_text` 時，`initialize()` 會以 `ALTER TABLE` 補欄；缺少人物分析表時則以 `CREATE TABLE IF NOT EXISTS` 補建。這類附加式升級會保留既有索引列，`body_visible_text` 留空的舊列之後在首次開啟人物相關功能時分批補建。

完成上述升級後，`hasExpectedIndexSchema()` 才依必要欄位與已移除欄位判斷相容性。其餘缺欄或仍含 `mood`、`schema_version` 等舊痕跡時，才視為需要重置索引。
...
| schema 升級與加密連線測試 | [`index_encrypted_connection_test.dart`](../../../test/infrastructure/database/index_encrypted_connection_test.dart) |
| 索引可用性測試 | [`ensure_index_ready_test.dart`](../../../test/infrastructure/storage/ensure_index_ready_test.dart) |
