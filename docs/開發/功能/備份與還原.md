...
- 依賴目前是 `file_picker: ^12.0.0-beta.7`（見 [`pubspec.yaml`](../../../pubspec.yaml)）。pub.dev 穩定最新仍是 `11.x`，`12.0.0` 正式版尚未發布。
- 留在 12 beta 是刻意取捨：Android 選檔常回傳 `content://`，materialize 路徑依賴 `AndroidPlatformFile` / `safHandle`（含 `releaseGrant()`），實作在 [`picked_file_materializer.dart`](../../../lib/infrastructure/storage/shared/picked_file_materializer.dart)。
- 預設不走整檔 bytes（`allowBytesFallback` 預設 `false`）。正式寫入外部資料夾在 Android 走自製 SAF channel，不依賴 `file_picker`。
- 穩定 `12.0.0` 出來後再升；屆時預期 `FilePickerResult` 等 API 還會再破一次，需一併改呼叫端。
- 不要為了「不用 beta」降回 11.x，除非 12 beta 出現明確回歸。
