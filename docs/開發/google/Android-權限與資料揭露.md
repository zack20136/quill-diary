...
- 遠端完成驗證前若使用者停止、系統逾時、服務或程序異常終止：先進入 `CANCEL_CLEANUP_PENDING` 並對帳遠端（已完成則記錄成功，否則清殘檔）；不另外建立失敗提示；對帳完成前不能開始新的上傳。細節見 [../功能/備份與還原.md](../功能/備份與還原.md)。
- 授權失效、重試耗盡或無法啟動前景服務等實際上傳失敗，仍會提示失敗，不跨程序續傳。
...
| dataSync FGS 額度 | 在 Android 15+ 裝置上，target Android 15（API 35）以上 App 的 dataSync 前景服務在背景每 24 小時合計最多約 6 小時 | 使用者把 App 帶回前景時系統會重設計時；額度用盡且尚未遠端驗證完成時進入 `CANCEL_CLEANUP_PENDING` 對帳，不另外提示失敗。細節見 [../功能/備份與還原.md](../功能/備份與還原.md)；不是每次上傳各自 6 小時 |
