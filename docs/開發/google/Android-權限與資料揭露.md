...
- 使用者主動停止時先中止傳輸並寫入 `CANCEL_CLEANUP_PENDING`，不建立 failure notice；若遠端已驗證完成（含驗證後、本機寫入前按下停止），會寫入並保留 `STATUS_PENDING` 或 `PRUNE_PENDING` 供 App 收尾；尚未通過遠端驗證時會在 worker 退出後（或下次啟動）查詢並刪除殘檔。
