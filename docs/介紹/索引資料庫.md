...
flowchart TD
    ...
    ensure --> schema{schema 或 lastRebuild 需重建?}
...
- 解鎖後才開啟
- 鎖定後就關閉
- 無法開啟、schema 落後或需要重建時，直接刪掉再重建

## `openForSession` 做什麼

`IndexDatabaseManager.openForSession` 負責把索引綁到目前的解鎖 session，**不**負責 `search_schema_version` 判斷。
...
1. 種入預設標籤目錄
2. 讀取 `lastRebuildAt` 與 `search_schema_version`
3. 若尚未重建過，或 schema 版本落後，呼叫 `rebuildIndex`
