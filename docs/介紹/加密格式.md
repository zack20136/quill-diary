...
flowchart TB
  subgraph vaultLayer [Vault 層]
    ...
    WRK -->|建立／輪替可信裝置時包裝| Store[Keystore + secure storage]
    Store -->|可信裝置 unwrap| Session[session 持有 recoveryWrapKey]
    WRK -->|輸入復原金鑰解鎖| Session
  end
  subgraph fileLayer [檔案層 LDJ2]
    FK[隨機 fileKey] --> AES[AES-256-GCM 正文]
    FK -->|包進 header| RS[recovery slot]
    Session -->|以 recoveryWrapKey 解開 slot| RS
    RS -->|解出| FK
  end
```
...
| 層級 | 金鑰 | 存放位置 | 用途 |
|------|------|----------|------|
| Vault 層 | `recoveryWrapKey` | 由復原金鑰衍生；可信裝置時以 `WrappedRecoveryKeyRecord` 同時存於 Android Keystore 與 `flutter_secure_storage` | 進入整個日記庫、解開各檔 recovery slot、衍生搜尋索引金鑰 |
...
2. `DeviceKeyManager.wrapWithDeviceKey` 把這把 key 包成 `WrappedRecoveryKeyRecord`，同時寫入 Android Keystore 與 `flutter_secure_storage`
3. 日常可信裝置解鎖時，`VaultRepository.openTrustedSessionEnsuringKeystore` 透過 Keystore unwrap 取得 `recoveryWrapKey`（必要時同步 Keystore 遷移）；內層再呼叫 `openTrustedSession` 完成 unwrap
...
可信裝置的本質是：先從 Keystore 與 secure storage 解開 vault 層的 wrapped recovery key，取得 `recoveryWrapKey`；之後讀寫各檔時，仍走檔案層 recovery slot。
...
1. `openTrustedSessionEnsuringKeystore` 從 secure storage 讀取包裝記錄，再透過 Keystore unwrap vault 層 wrapped recovery key
...
補充：使用復原金鑰解鎖或輪替時，若全庫 rewrap 中斷，索引可能留下 `rewrap_in_progress` 旗標，下次開啟會續跑。
