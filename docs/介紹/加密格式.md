...
`decryptBytes` 需要 `DecryptionContext` 提供 `recoveryWrapKey`（以及是否為可信裝置 session）。它不會在每個檔案裡尋找 per-file device slot 作為主要路徑。
...
3. 對目標檔案：從 header 的 recovery slot 解出 `fileKey`
4. 用 `fileKey` 解密正文

**復原金鑰路徑**

1. 使用者輸入復原金鑰，經 Argon2id 衍生 `recoveryWrapKey`
2. 對目標檔案：從 header 的 recovery slot 解出 `fileKey`
3. 用 `fileKey` 解密正文
