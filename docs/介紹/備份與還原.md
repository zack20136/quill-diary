...
5. `finishRestoreSession` 重置依賴並啟動還原後 session
...
| 同 `vaultId`、同 recovery generation、本機仍有可信裝置，且還原前 session 仍有效 | `resumeSessionAfterRestore`：沿用還原前 session，**免再次螢幕鎖 / 生物驗證** |
| 備份與本機可信狀態仍對應，但無法直接沿用 session | 進入 `recoveryRequired`，要求輸入復原金鑰 |
