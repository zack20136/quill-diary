...
啟動、背景逾時、本機受信任裝置還原，以及三種解鎖模式與 `ResumeUnlockAction` 行為。
...
| `KeystoreAuthKind` | 用途 |
|--------------------|------|
| `plain` | 無額外驗證；`none` 模式本機受信任 unwrap |
...
| **`VaultRepository`** | 本機受信任 session、`syncDeviceCredentialBackupWrapIfNeeded`、`ensureKeystoreMatchesUnlockMode` |
...
- 本機受信任狀態失效、還原後日記庫不符、或 legacy slot → `recoveryRequired`，須輸入復原金鑰。
